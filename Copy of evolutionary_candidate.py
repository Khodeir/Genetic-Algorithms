...
class candidate(object):
    ...
    def __init__(self, binrep):
        '''Create an individual candidate with binary representation binrep.'''
    ...
    def mate_crossover(self, mate):
        ...
        childbinrep = ''
        geneLength= len(self.bin_rep)
        crossover= random.randrange(0, geneLength+1)
        for i in range(crossover):
            childbinrep += self.bin_rep[i]
        for i in range(geneLength-crossover):
            childbinrep += mate.bin_rep[crossover+i]
        offspring= candidate(childbinrep)
        return offspring
